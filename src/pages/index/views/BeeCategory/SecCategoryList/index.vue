    ...
    <div class="container position-ab-top-46">
      ...
      <van-tabs v-model="nowThirdIndex" swipeable @click="getCateId">
        ...
        <div class="show-all-cate" @click="showAll">
          <img src="@/assets/category/list_icon_more.png" alt="">
        </div>
        <!-- 全部分类列表 -->
        <div v-show="showAllCate" class="all-cate">
          <div class="title flex flex-between align-center">
            <span>全部分类</span>
            <div class="collapse" @click="showAllCate=false">
              <img src="@/assets/category/list_icon_collapse.png">
            </div>
          </div>
          <ul class="cate-list flex flex-wrap">
            <li v-for="(item,index) in categoryThirdList" :key="index">
              <button class="cate-btn" :class="{active: index==condition.nowThirdId}" type="button" @click="getCateIdFromAll(index,item.name)">
                {{ item.name }}
              </button>
            </li>
          </ul>
        ...
        <!-- tabs 标签 -->
        <van-tab v-for="(item,index) in categoryThirdList" :key="index" :title="item.name">
          <!-- 筛选条件 -->
          <FilterBox :type="condition.nowType" @getFilter="getFilter" @showWay="showWay" />
          <!-- 商品列表 -->
          <!-- 垂直一栏布局 -->
          <VerticalList v-show="isVertical" ref="VerticalList" :condition="condition" />
          <!-- 两栏布局 -->
          <HorizontalList v-show="!isVertical" ref="HorizontalList" :condition="condition" />
        </van-tab>
      </van-tabs>
    </div>
    <!-- 遮罩 -->
  ...
  methods: {
    ...
    getThird(id) {
      getCategoryThird(id).then(res => {
        console.log(res)
        this.categoryThirdList = res.data.categoryThirdList
      })
    },
    // 获取商品列表
...
.sec-category{
  ...
  .show-all-cate{
    ...
    width:1rem;
    display: flex;
    background-image: linear-gradient(to right,rgba(245, 245, 245, 0.7),rgb(245, 245, 245,1));
    img{
      margin: auto;
      width: 0.24rem;
      height: 0.13rem;
  ...
  .all-cate{
    ...
    .title{
      ...
      font-size: 0.28rem;
      color: #666;
      .collapse{padding: 0.2rem;}
      img{
        width: 0.24rem;
        height: 0.13rem;
      }
    }
    li{margin-right: 0.2rem; margin-bottom: 0.2rem;}
    .cate-btn{
